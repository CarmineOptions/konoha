...
mod streaming {
    ...
    > of super::IStreaming<ComponentState<TContractState>> {
        ...
        fn claim_stream(
            ...
            let (already_claimed, total_amount, is_minting): (u128, u128, bool) = self
                .streams
                .read(key);
        ...
        fn get_stream_info(
            ...
            let key: (ContractAddress, ContractAddress, u64, u64) = (
                streamer, recipient, start_time, end_time
            );
            let (claimable_amount, total_amount, is_minting): (u128, u128, bool) = self
                .streams
                .read(key);
            (claimable_amount, total_amount, is_minting)
        }
    }
}
