...
1. vest(grantee: ContractAddress, vested_timestamp: u64): This function executes the vesting process for a specific grantee at a given timestamp.
...
# Executing Vesting
To execute the vesting process for a grantee at a specified timestamp, use the vest function. This function will check if the conditions are met and, if so, mint the vested tokens to the grantee.
...
* vested_timestamp: The timestamp of the vesting event.
...
The Vesting contract is composed of the following components:
...
For additional customization possibilities such as options on the governance token, further modifications can be explored within the contract.