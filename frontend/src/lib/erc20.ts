export function formatBalance(balance: bigint | undefined): string {
    ...
    if (ether >= 1) {
        // For values >= 1, show all digits before decimal and 2 after
        return ether.toFixed(2);
    } else {
        // For values < 1, use toSignificantDigits
        return toSignificantDigits(ether, 2);
    }
}

function toSignificantDigits(num: number, significantDigits: number): string {
    if (num === 0) return "0";

    const parts = num.toExponential(significantDigits - 1).split("e");
    ...
    if (exponent >= 0) {
        ...
        const decimalPlaces = Math.abs(exponent) - (coefficient.includes(".") ? 1 : 0);
