...
fn test_locking_sequence() {
    ...
    let floating_token_dispatcher = IERC20Dispatcher {
        contract_address: floating_token.contract_address
    };
    ...
    setup_staking(
        gov.contract_address, floating_token.contract_address, voting_token.contract_address
    );
    ...
    assert_eq!(
        admin_balance_after, admin_balance - amount.into(), "Incorrect balance after locking"
    );
    ...
    assert_eq!(
        admin_balance_after,
        admin_balance - (amount.into() + increase_amount.into()),
        "Incorrect balance after increasing lock"
    );
    ...
    assert_eq!(
        initial_locked_end,
        get_block_timestamp() + lock_duration,
        "Unlock time should be 1 year from now"
    );
    ...
    assert_eq!(
        new_locked_amount, amount + increase_amount, "Locked amount should remain the same (6k)"
    );
    ...
    assert_eq!(
        admin_balance_after,
        admin_balance - (amount.into() + increase_amount.into()),
        "Balance should remain the same after extending lock duration"
    );

    start_warp(
        CheatTarget::One(gov.contract_address), new_unlock_date + 1
    ); // Warp time to just after the new unlock date
    ...
    assert_eq!(
        admin_balance_after_withdraw, admin_balance, "Balance should be restored after withdrawal"
    );
...
fn test_total_supply() {
    ...
    assert_eq!(
        ...
        "Initial total supply should be equal to locked amounts"
    );
